...
def register_user(
    ...
    *,
    ...
    if auth_repo.get_user_by_email(db, normalized_email):
        raise AppError("EMAIL_ALREADY_EXISTS", "Email is already registered", status_code=409)
    if auth_repo.get_user_by_username(db, username):
        raise AppError("USERNAME_ALREADY_EXISTS", "Username is already taken", status_code=409)
...
def login_user(
    ...
    *,
    ...
    return _token_pair_response(user.id, refresh_token)
...
def refresh_tokens(
    ...
    *,
    ...
    if token_row.expires_at < now or token_row.revoked_at is not None:
        raise AppError("INVALID_REFRESH_TOKEN", "Invalid refresh token", status_code=401)

    return AccessTokenResponse(access_token=create_access_token(str(token_row.user_id)))
...
def _token_pair_response(user_id: int, refresh_token: str) -> TokenPairResponse:
